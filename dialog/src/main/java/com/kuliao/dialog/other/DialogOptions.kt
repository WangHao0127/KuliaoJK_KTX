package com.kuliao.dialog.other
...
import com.kuliao.dialog.R
import com.kuliao.dialog.extensions.UtilsExtension.Companion.unDisplayViewSize
...
open class DialogOptions() : Parcelable {
     ...
     */
    var dialogThemeFun: (commonDialog: CommonDialog) -> Int = {
        it.dialogActivity!!.run {
            ...
            (this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                    ...
                    || this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR).yes {
                R.style.GenjiDialogFullScreen
            }.otherwise {//非沉浸式状态栏（包括普通的带状态栏页面以及全屏无状态栏页面）
                R.style.GenjiDialog
            }
        }
    }
     ...
     * dataBinding绑定
     */
    var bindingListener: ((container: ViewGroup?, dialog: CommonDialog) -> View)? = null
     ...
     * 该方法对象可重写
     */
    var setStatusBarModeFun: (commonDialog: CommonDialog) -> Unit = { commonDialog ->
        (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP).yes { //5.0 全透明实现
            commonDialog.dialogActivity.apply {
                //如果activity是否是占满全屏并且依旧保留状态栏（沉浸式状态栏）
                (this!!.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                        || this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
                        || this.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR).yes {
                    ...
                    (findViewById<View>(android.R.id.content) as ViewGroup).getChildAt(0).fitsSystemWindows.let {
                        //（与activity保持一致）
                        commonDialog.dialog?.window?.decorView?.fitsSystemWindows = it
                        it.yes { //无论是否预留了statusBar，都将dialog设置可占满全屏
                            commonDialog.dialog?.window?.decorView?.systemUiVisibility =
                                View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
                            //此时可以设置dialogStatusBarColor
                            //commonDialog.dialog.window.statusBarColor = dialogStatusBarColor
                        }.otherwise { //如果不预留statusrBar，那么dialog设置可占满全屏
                            commonDialog.dialog?.window?.decorView?.systemUiVisibility =
                                this.window.decorView.systemUiVisibility
                        }
                    }

                }.otherwise { //非沉浸式状态栏（包括普通的带状态栏页面以及全屏无状态栏页面）此时的statusBarColor不生效
                    commonDialog.dialog?.window?.decorView?.systemUiVisibility =
                        View.SYSTEM_UI_FLAG_VISIBLE
                }
            }
        }.otherwise { //4.4 全透明状态栏
            commonDialog.dialog?.window?.addFlags(commonDialog.dialogActivity!!.window.attributes.flags)
        }
    }
     ...
     */
    fun setDialogTheme(function: (commonDialog: CommonDialog) -> Int) {
     ...
     */
    fun setStatusMode(function: (commonDialog: CommonDialog) -> Unit) {
    ...
    fun dialogAsView(
        view: View,
        gravityAsView: DialogGravity = this.gravityAsView,
        @StyleRes newAnim: Int? = this.animStyle,
        ...
        when (gravityAsView) {
            ...
            DialogGravity.LEFT_TOP -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX100Y100
            ...
            DialogGravity.CENTER_TOP -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX50Y100
                this.dialogViewX =
                    viewX - ((if (width != 0) width else dialogViewWidth) - viewWidth) / 2 + offsetX
                this.dialogViewY = viewY - if (height != 0) height else dialogViewHeight + offsetY
            }
            //dialog显示在view的右上角
            DialogGravity.RIGHT_TOP -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX0Y100
            ...
            DialogGravity.LEFT_CENTER -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX100Y50
                this.dialogViewX = viewX - if (width != 0) width else dialogViewWidth + offsetX
                this.dialogViewY =
                    viewY - ((if (height != 0) height else dialogViewHeight) - viewHeight) / 2 + offsetY
            }
            //dialog显示在view的正中心
            DialogGravity.CENTER_CENTER -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX50Y50
                this.dialogViewX =
                    viewX - ((if (width != 0) width else dialogViewWidth) - viewWidth) / 2 + offsetX
                this.dialogViewY =
                    viewY - ((if (height != 0) height else dialogViewHeight) - viewHeight) / 2 + offsetY
            ...
            DialogGravity.RIGHT_CENTER -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX0Y50
                this.dialogViewX = viewX + viewWidth + offsetX
                this.dialogViewY =
                    viewY - ((if (height != 0) height else dialogViewHeight) - viewHeight) / 2 + offsetY
            }
            ...
            DialogGravity.LEFT_BOTTOM -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX100Y0
                this.dialogViewX = viewX - if (width != 0) width else dialogViewWidth + offsetX
            ...
            DialogGravity.RIGHT_BOTTOM -> {
                if (this.animStyle == 0) this.animStyle =
                    R.style.ScaleOverShootEnterExitAnimationX0Y0
                this.dialogViewX = viewX + viewWidth + offsetX
                this.dialogViewY = viewY + viewHeight + offsetY
            }
            else -> {
    ...
    companion object CREATOR : Parcelable.Creator<DialogOptions> {
        override fun createFromParcel(parcel: Parcel): DialogOptions {
            return DialogOptions(parcel)
        }

        override fun newArray(size: Int): Array<DialogOptions?> {
