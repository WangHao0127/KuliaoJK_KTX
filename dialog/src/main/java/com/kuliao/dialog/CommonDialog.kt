...
import com.kuliao.dialog.extensions.AnimatorListenerEx
import com.kuliao.dialog.extensions.UtilsExtension.Companion.getScreenHeight
import com.kuliao.dialog.extensions.UtilsExtension.Companion.getScreenHeightOverStatusBar
import com.kuliao.dialog.extensions.UtilsExtension.Companion.getScreenWidth
import com.kuliao.dialog.listener.OnKeyListener
import com.kuliao.dialog.other.DialogGravity
import com.kuliao.dialog.other.DialogInitMode
...
open class CommonDialog : DialogFragment() {
    ...
    override fun onStart() {
        ...
        initParams()
        //初始化配置
        (dialogOptions.unLeak && isDismissed == null).yes {
    ...
    fun setDialogOptions(dialogOptions: DialogOptions): CommonDialog {
...
//            dataConvertView(dialogBinding!!, this)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //如果是继承了CommonDialog，那么就会重写extendsOptions()方法
     ...
     * 数据绑定到视图/视图控件监听等
     */
    private fun convertView(holder: ViewHolder, dialog: CommonDialog) {
     ...
     */
    private fun dataConvertView(dialogBinding: Any, dialog: CommonDialog) {
    ...
    override fun dismiss() {
        ...
        dialogOptions.exitAnimator.apply {
            ...
            (dismissed.compareAndSet(false, true)).yes {
    ...
    override fun onStop() {
        ...
        (dismissed.compareAndSet(true, false)).yes {
            return
        }
        //当防止内存泄露为false的时候才执行
        (!dialogOptions.unLeak).yes {
    ...
    private fun executeShowListener() {
        for (entry in dialogOptions.showDismissMap.entries) {
            (entry.value.enableExecuteShowListener).yes {
    ...
    private fun executeDismissListener() {
        for (entry in dialogOptions.showDismissMap.entries) {
            (entry.value.enableExecuteDismissListener).yes {
    ...
    private fun initParams() {
        ...
        dialog?.window?.let { window ->
            ...
            dialogOptions.setEnterAnimatorFun?.invoke(window.decorView.findViewById(android.R.id.content))
                ?.let {
                    it.addListener(initAnimatorEnterListener())
                    dialogOptions.enterAnimator = it
                }
            //设置dialog隐藏时，布局中view的自定义动画
            dialogOptions.setExitAnimatorFun?.invoke(window.decorView.findViewById(android.R.id.content))
                ?.let {
            ...
            window.attributes = window.attributes?.apply {
                ...
                //当左右占满时，设置左右两边的平均边距
                (dialogOptions.isFullHorizontal).yes {
                    horizontalMargin = 0f
                ...
                (dialogOptions.isFullVertical).yes {
                    verticalMargin = 0f
                ...
                //（包含StatusBar）真正的全屏
                (dialogOptions.isFullVerticalOverStatusBar).yes {
                    verticalMargin = 0f
                    height =
                        getScreenHeightOverStatusBar(dialogActivity!!) - 2 * dialogOptions.fullVerticalMargin
                }
                //设置位置(如果设置了asView,那么gravity则永远为LEFT_TOP)
                gravity = dialogOptions.gravity.index
                //如果设置了asView，那么设置dialog的x，y值，将dialog显示在view附近
                (dialogOptions.isAsView()).yes {
     ...
     * @param manager
     */
    fun showOnWindow(manager: FragmentManager): CommonDialog {
        myIsShow = true
        dialogOptions.apply {
     ...
     */
    fun showOnWindow(manager: FragmentManager, gravity: DialogGravity = dialogOptions.gravity): CommonDialog {
        myIsShow = true
        dialogOptions.apply {
     ...
     */
    fun showOnWindow(manager: FragmentManager, gravity: DialogGravity = dialogOptions.gravity, @StyleRes newAnim: Int? = dialogOptions.animStyle): CommonDialog {
        myIsShow = true
        dialogOptions.apply {
            this.gravity = gravity
     ...
     * @param tag
     */
    fun showOnWindow(manager: FragmentManager, gravity: DialogGravity = dialogOptions.gravity, @StyleRes newAnim: Int? = dialogOptions.animStyle, tag: String?): CommonDialog {
        myIsShow = true
        dialogOptions.apply {
            this.gravity = gravity
            this.animStyle = newAnim
     ...
     * @param tag
     */
    fun showNowOnWindow(manager: FragmentManager, gravity: DialogGravity = dialogOptions.gravity, @StyleRes newAnim: Int? = dialogOptions.animStyle, tag: String?): CommonDialog {
        myIsShow = true
        dialogOptions.apply {
            this.gravity = gravity
            this.animStyle = newAnim
     ...
     * @param view 被依赖的view
     */
    fun showOnView(manager: FragmentManager, view: View): CommonDialog {
        myIsShow = true
     ...
     */
    fun showOnView(manager: FragmentManager, view: View, gravityAsView: DialogGravity = dialogOptions.gravityAsView): CommonDialog {
        myIsShow = true
     ...
     */
    fun showOnView(manager: FragmentManager, view: View, gravityAsView: DialogGravity = dialogOptions.gravityAsView, @StyleRes newAnim: Int? = dialogOptions.animStyle): CommonDialog {
        myIsShow = true
     ...
     */
    fun showOnView(manager: FragmentManager, view: View, gravityAsView: DialogGravity = dialogOptions.gravityAsView, @StyleRes newAnim: Int? = dialogOptions.animStyle, offsetX: Int = dialogOptions.offsetX, offsetY: Int = dialogOptions.offsetY): CommonDialog {
        myIsShow = true
